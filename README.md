...
- `dangerous` - annotate sequences that would be difficult to work with. Currently this includes:
      1. `IN_GENOME=X`: The number of times a perfect match target for this guide sequence is seen within the genome of interest. 
      2. `GC_X`: flagging sequences that have a high (>75%) or low (<25%) GC content
      3. `PolyT`: guide sequences that have four or more thymine (T) bases in a row. Could potentially terminate polIII transcription early (not an issue with other transcription approaches)

...
import standards.{CAS9, Cas9ParameterPack, ParameterPack}
...
class Doench2014OnTarget extends ScoreModel with LazyLogging {
  ...
  /**
    ...
    */
  override def validOverScoreModel(enzyme: ParameterPack): Boolean = enzyme.name match {
    case _ : CAS9.type => true
    case _ => false
  }
  ...
  override def validOverGuideSequence(enzyme: ParameterPack, guide: CRISPRSiteOT): Boolean = {
    require(enzyme.name == CAS9,"We cannot score this guide, as the target wasn't screened with Cas9") // this should already by checked but check again
